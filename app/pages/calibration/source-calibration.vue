<template>
  <div class="bg-surface text-on-surface font-body selection:bg-primary-container selection:text-on-primary-container min-h-screen relative z-0">
    
    <div class="fixed top-0 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-primary-container/5 blur-[120px] rounded-full -z-10 pointer-events-none"></div>
    <div class="fixed bottom-0 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-secondary-container/5 blur-[120px] rounded-full -z-10 pointer-events-none"></div>

    <header class="fixed top-0 left-0 w-full z-50 bg-[#131313]">
      <div class="mx-auto w-full max-w-4xl flex items-center justify-between px-6 py-4">
    ...
    <main class="mt-12 pt-6 pb-2 px-6 max-w-4xl mx-auto flex flex-col">
      ...
      <section>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div v-for="(source, index) in sources" :key="index" class="bg-surface-container-low p-5 rounded-xl flex flex-col justify-between hover:bg-surface-container transition-colors group border border-outline-variant/10">
            <div class="flex justify-between items-start mb-3">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-surface-container-highest rounded-full flex items-center justify-center text-primary-container">
              ...
              <button @click="promptDelete(index)" :disabled="isSaving" class="text-on-surface-variant hover:text-error transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
                <span class="material-symbols-outlined text-xl">delete</span>
              </button>
            </div>
            
            <div class="flex flex-wrap gap-2 mt-2">
              <span v-for="(badge, bIndex) in getBadges(source.status)" :key="bIndex" :class="['inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-label outline outline-1', badge.classes]">
      ...
      <section class="flex flex-col items-end pt-2">
        <p v-if="saveErrorMsg" class="text-error text-[12px] font-label mb-3 leading-tight text-red-500 text-right">
          {{ saveErrorMsg }}
        </p>
        <button
...
// Most már komplex objektumokat tárolunk az ellenőrzés állapotával!
