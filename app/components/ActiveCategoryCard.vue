<template>
  <div 
    class="glass-card rounded-2xl p-6 flex flex-col space-y-3 border-2 transition-all relative" 
    :style="{ 
  ...
  >
    ...
    <div class="space-y-4">
      ...
      <div class="relative pt-6">
        ...
        <div class="text-[10px] text-on-surface-variant/60 font-medium text-right mt-1 pr-1">
          {{ localPrompt.length }}/500
        </div>
      </div>
      
      <p class="text-[11px] text-on-surface-variant italic opacity-70">AI-curated sub-nodes based on calibration.</p>
      
      <div class="flex flex-wrap gap-2 transition-all duration-300">
        <span v-for="chip in visibleChips" :key="chip" class="bg-surface-container-highest text-primary-fixed-dim px-3 py-1 rounded-full text-[10px] font-medium border border-outline-variant/10">
          {{ chip }}
        </span>
      ...
      <button 
        v-if="hasChanges"
        @click="saveLocal"
        class="w-full mt-4 py-2 bg-[#fec931] text-on-tertiary-fixed font-bold rounded-lg text-xs uppercase tracking-widest hover:bg-[#fec931]/90 transition-all active:scale-[0.98]"
      >
        Save Changes
      </button>
    </div>
