# Agent 5 - Lifecycle, retention, and advanced recommendation layer

## Dev Plan

## 1. Why Agent 5 should be separate

The original Agent 4 scope became too large. Combining scoring MVP work with long-term content lifecycle handling would create too many responsibilities in a single layer.

That is why Agent 5 should be treated as a separate layer for:
...
If Agent 4 only handles scoring, then:

- optimization is simpler
- operations are cheaper
- debugging is easier

As a separate layer, Agent 5:
...
These rules should not be forced into the Agent 4 MVP.
...
Agent 5 should not run in the request path.
...
Separating Agent 5 is justified.

This keeps the system clean:

1. Agent 1: discovery + ingest
2. Agent 3: enrichment
3. Agent 4: scoring + feed ordering
4. Agent 5: lifecycle + retention + advanced recommendation
