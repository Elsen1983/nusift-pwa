# Agent 2 - Article enrichment and content extraction
...
2. **Agent 2**: article-level enrichment on already stored candidate/article records
3. **Agent 3**: per-user relevance scoring and feed ordering
4. **Agent 4**: lifecycle, retention, archive/prune, and later social / advanced recommendation

Agent 2 is **not** a feed discovery agent. It does not work on source URLs. It builds on the data already collected by Agent 1:
...
- Agent 2 should not rely only on `Article.title` / `Article.bodyText`
- Agent 2 should also use the evidence stored by Agent 1
...
- Because of that, the final article-level paywall state should be re-evaluated per article by Agent 2.
...
So Agent 2 should not be designed as if the system were still only a simple RSS -> Article insert flow.

### 4.1 Explicit upstream Agent 1 behavior

Current Agent 1 is more than fetch-based feed discovery.

Verified upstream elements that Agent 2 must respect:
...
This matters for Agent 2 because the article universe may not be complete after the first ingest pass. Later hard-case queue resolution and targeted reruns can bring in additional relevant articles for the same source/category universe.

## 5. Data model expectations

Agent 2 will likely need additional fields on the `Article` model:
...
The first version of Agent 2 should **not** be forced to run inline immediately after Agent 1.

Better flow:

1. Agent 1 runs
2. it stores articles and artifacts
3. Agent 2 selects fresh articles that are suitable for enrichment
...
Agent 2 should only run on articles that:
...
Agent 2 selection logic must be compatible with Agent 1's hard-case resolution reruns.

Therefore:

- Agent 2 should not think only in terms of the "last pipeline run"
...
Agent 2 works correctly only if articles added during hard-case reruns are automatically eligible for enrichment.
...
Agent 2 should determine on a per-article basis whether the given URL is actually paywalled.
...
1. Agent 2 attempts HTTP extraction for the article
...
Agent 2 artifact payload should not hide the upstream discovery context coming from Agent 1.
...
- preparation for Agent 3, but not ranking and not lifecycle

Agent 2 will only be stable if it explicitly builds on the Agent 1 artifact / raw / normalized / provenance logic and remains compatible with the hard-case browser fallback + targeted rerun behavior.
