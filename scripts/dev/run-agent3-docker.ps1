<#
.SYNOPSIS
    Run the Agent 3 enrichment batch runner inside a Docker container that mirrors
    the production Vercel Linux runtime (playwright-core + @sparticuz/chromium).

.DESCRIPTION
    Builds the Dockerfile.agent2-dev image if needed, then runs the Agent 3
    batch runner script inside the container. Reuses the same browser-capable
    Docker image as Agent 2.

    All CLI arguments are forwarded to the runner (e.g. --maxArticles,
    --browserFallback, --browserFallbackMaxAttempts).
...
.PARAMETER batch
    Run the bounded Agent 3 batch processor.

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File scripts/dev/run-agent3-docker.ps1 --maxArticles=10 --browserFallback=true --browserFallbackMaxAttempts=3

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File scripts/dev/run-agent3-docker.ps1 -batch --maxArticles=5 --includeEnriched=false --forceReprocess=false
#>
...
# ── Docker image name (reuse Agent 2 image) ────────────────────────────────
...
$cmd = @("scripts/dev/run-agent3-batch.ts") + $RemainingArgs
...
# ── Run ───────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "Running Agent 3 in Docker..." -ForegroundColor Green
