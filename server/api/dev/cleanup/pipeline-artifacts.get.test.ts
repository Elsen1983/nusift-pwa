...
describe("GET /api/dev/cleanup/pipeline-artifacts", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  async function loadHandler() {
    const mod = await import("./pipeline-artifacts.get");
    return mod.default;
  }
  ...
  it("calls assertRateLimit with cleanup-pipeline-artifacts-inspect / 10 / 60s", async () => {
    const handler = await loadHandler();
    await handler({} as any);
    expect(mockAssertRateLimit).toHaveBeenCalledWith(
      expect.anything(),
      "cleanup-pipeline-artifacts-inspect",
      10,
      60 * 1000,
    );
  });

  it("always calls the utility with dryRun=true (ignores query param)", async () => {
  ...
  it("parses and clamps limit (1..1000)", async () => {
