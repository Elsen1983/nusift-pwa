...
const FEED_LINK_REGEX =
  /<link[^>]+rel=["'][^"']*alternate[^"']*["'][^>]+type=["'](application\/(?:rss\+xml|atom\+xml|feed\+json)|application\/json)["'][^>]+href=["']([^"']+)["'][^>]*>/gi;
...
type FeedCandidate = {
  ...
  detection: "direct-feed" | "html-link" | "http-link" | "robots-sitemap" | "cms-fingerprint";
...
const resolveHtmlDeclaredFeeds = (body: string, pageUrl: string) => {
  ...
  for (const match of body.matchAll(FEED_LINK_REGEX)) {
    const contentType = normalizeSupportedContentType(match[1] || null);
    const resolved = resolveRelativeUrl(match[2] || "", pageUrl);
...
const looksLikeFeedUrl = (url: string) => {
  try {
    ...
    return (
      ...
      normalized.includes("feed=") ||
      normalized.includes("format=rss") ||
      normalized.includes("format=atom") ||
